...
export function getMockState(scenario: Scenario) {
  const presets: Record<Scenario, { risk: number; state: string; decision: string; robot: string; accent: string; people: number; a: number; b: number; ripple: boolean }> = {
    ...
    ripple: { risk: 79, state: 'RIPPLE DETECTED', decision: 'Hold perimeter · dampen flow', robot: 'Beacon pattern active', accent: 'red', people: 431, a: 67, b: 63, ripple: true },
  ...
  const robot: RobotAction = { arm: p.risk > 75 ? 'ACTIVE' : 'STANDBY', display: p.robot, exitALights: p.a > p.b ? 'RED · HOLD' : 'GREEN · OPEN', exitBLights: p.b > p.a ? 'RED · HOLD' : 'GREEN · OPEN', audio: p.risk > 75 ? 'ROUTE_B_03' : 'QUIET', beacon: p.ripple ? 'PULSE · 4 Hz' : 'OFF' };
